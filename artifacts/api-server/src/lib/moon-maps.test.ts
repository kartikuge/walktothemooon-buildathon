...
// All writes, including backfill sequence updates, target temporary shadow
// tables/private identities. No application rows or sequences are mutated.
test("additive backfill, zero-mile enrollment, isolated completion and replay",async()=>{
  ...
  await c.query("BEGIN");
  try {
    const serialTables=["moon_users","moon_routes","moon_teams","moon_members","moon_competitions","moon_competition_teams","moon_maps","moon_map_completions","moon_runs","moon_stamps"];
    for(const table of [...serialTables,"moon_settings","moon_activity","moon_map_previews"]) {
      await c.query(`CREATE TEMP TABLE ${table} (LIKE public.${table} INCLUDING ALL) ON COMMIT DROP`);
    ...
    await c.query("INSERT INTO moon_settings(id,historical_miles) VALUES(1,0)");
    await c.query("INSERT INTO moon_competitions(id,name,route_id,end_date) VALUES(1,'Isolated fixture competition',1,'2099-12-31')");
